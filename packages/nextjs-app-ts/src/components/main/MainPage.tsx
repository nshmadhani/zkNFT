...
import { StakeApp } from '../page/StakeApp';
...
export const MainPage: NextPage<IMainPageProps> = (props) => {
  ...
  // init contracts
  const proveNFT = useAppContracts('ProveNFT', ethersAppContext.chainId);
  const yourNFT = useAppContracts('MockERC721', ethersAppContext.chainId);
  // const mainnetDai = useAppContracts('DAI', networkDefinitions.mainnet.chainId);

  // keep track of a variable from the contract in the local React state:
  const [groupId, update] = useContractReader(
    proveNFT,
    proveNFT?.groupId,
    [],
  );
  ...
  const [setPurposeEvents] = useEventListener(proveNFT, 'SetPurpose', 0);
